...
# Step 1: Get a list of all ".gpkg" files in the specified directory
all = list.files(wb_dir, full.names = TRUE, pattern = ".gpkg$")

# Step 2: Loop through each ".gpkg" file in the directory and process water body features
for(i in 1:length(all)){
  
  # Step 3: Get the path of the current file
  path = all[i]
  
  # Step 4: Get the VPU name from the file name
  which_VPU = gsub(".gpkg", "", gsub("NHDPlus", "", basename(path)))
  
  # Step 5: Specify the output file name based on the VPU name
  outfile = glue("{reference_dir}waterbodies_{which_VPU}.gpkg")
  
  # Step 6: Only process the file if the output file doesn't exist yet
  if(!file.exists(outfile)){
    
    # Step 7: Read in the water body data from the current file as an sf object
    wb = read_sf(path)
    
    # # Step 8: Drop the geometry column and rename column names to upper case
    ...
    # Step 9: Union polygons by the COMID attribute and transform to EPSG code 5070
    xx =
      wb %>% 
    ...
    # Step 10: Filter rows where GNIS_NAME is NA
    xx1 = 
      xx %>% 
      filter(is.na(GNIS_ID)) %>%
      mutate(member_comid = as.character(COMID))
    
    # Step 11: Filter rows where GNIS_NAME is not NA and add a new area_sqkm column
    xx2 =
      xx %>% 
      filter(!is.na(GNIS_ID)) %>%
    ...
    if(nrow(xx2) > 0) {
      ...
      # # Step 12: Union polygons by the GNIS_NAME attribute and join with xx2 data
      out = 
        xx2 %>% 
        union_polygons("GNIS_ID") %>%
        left_join(
          st_drop_geometry(xx2), 
          by = "GNIS_ID"
        ) %>%
        group_by(GNIS_ID) %>%
    ...
    # Step 13: Write the resulting data to a new ".gpkg" file
    write_sf(out, outfile, "waterbodies")
  }
  
  # Logging message indicating which VPU has finished processing its water bodies
